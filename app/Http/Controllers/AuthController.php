...
class AuthController extends Controller
{
    ...
    public function postRegister(Request $request) {
        $request->validate([
            ...
            'terms' => 'required',
            'role' => 'required|in:student,caretaker'
        ], [
        ...
        // Determine the user role ID based on the selected role
        $role_id = $request->role === 'student' ? 1 : 3;
        ...
        $user = User::create([
            ...
            'user_role_id' => $role_id // Assign the role ID
        ]);
    
        Mail::to($request->email)->send(new EmailVerificationMail($user));
    
        return redirect()->back()->with('success', 'Registration successful. Please check your email address for the email verification link.');
    }
